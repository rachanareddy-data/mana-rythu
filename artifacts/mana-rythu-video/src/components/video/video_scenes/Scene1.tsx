  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center text-center">
        ...
        >
          <img src={`${import.meta.env.BASE_URL}images/sprout.png`} alt="Sprout" className="w-24 h-24 mb-6 drop-shadow-2xl" />
          <h1 className="text-[7vw] font-black text-white tracking-tighter leading-none">
            Mana Rythu
          </h1>
        </motion.div>

        <motion.p
          className="text-[2.5vw] font-bold text-[#22c55e] mt-4"
        ...
        <div className="flex flex-wrap justify-center gap-4 mt-12 px-8">
          {pills.map((pill, i) => (
            <motion.div
              key={pill.text}
              className="flex items-center gap-2 bg-white/10 backdrop-blur-sm border border-white/20 px-6 py-3 rounded-full"
            ...
            >
              ...
              <span className="text-xl font-bold text-white">{pill.text}</span>
            </motion.div>
          ))}
        </div>

        <motion.div
          className="absolute top-[35vh] text-[1.5vw] text-white/80 font-medium tracking-wider"
